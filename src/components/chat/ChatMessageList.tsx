            ...
            >
              {canDelete && (
                <button
                  ...
                  className="absolute right-2 top-2 flex h-6 w-6 items-center justify-center rounded-lg text-navy-800/30 hover:bg-white hover:text-mx-red-600"
                >
          ...
          <div key={m.id} className={`flex flex-col ${isMine ? "items-end" : "items-start"}`}>
            ...
            <div className={`flex items-center gap-3 px-1 ${canDelete || canModerate ? "mt-1" : ""}`}>
              {canDelete && (
                <button
                  onClick={() => onDeleteMessage(m.id)}
                  className="flex items-center gap-1 text-[11px] font-medium text-navy-800/40 hover:text-mx-red-600"
                >
              ...
              {canModerate && !isMine && m.author.role !== "MODERATOR" && (
                blockedAuthorIds.has(m.author.id) ? (
                  <span className="text-[11px] font-semibold text-mx-red-600">
