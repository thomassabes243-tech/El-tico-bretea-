...
export async function POST() {
  ...
  await cancelPaypalSubscription(company.paypalSubscriptionId, "Cancelado por el usuario").catch(() => undefined);
  await prisma.companyProfile.update({
    where: { userId: session.user.id },
    data: { professionalPlanActive: false, paypalSubscriptionId: null },
  });
