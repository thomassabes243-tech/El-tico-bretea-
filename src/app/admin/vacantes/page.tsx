...
import { toggleJobPostingActive, toggleJobPostingFeatured } from "./actions";
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-navy-800/60">
        {jobPostings.length} vacantes publicadas por todas las empresas. &ldquo;Destacar&rdquo;
        marca la vacante como Brete Premium en Inicio -- es curación editorial, no algo que
        la empresa pueda activar por su cuenta.
      </p>
          ...
          <Card key={job.id} className="flex flex-wrap items-center gap-3 p-3.5">
            ...
            <div className="min-w-0 flex-1">
              <p className="flex items-center gap-1.5 truncate text-sm font-semibold text-navy-900">
                {job.title}
                {job.isFeatured && <Star className="h-3.5 w-3.5 shrink-0 fill-gold-600 text-gold-600" />}
              </p>
            ...
            <form
              action={async () => {
                "use server";
                await toggleJobPostingFeatured(job.id, !job.isFeatured);
              }}
            >
              <button
                type="submit"
                className={`flex shrink-0 items-center gap-1.5 rounded-lg border px-2.5 py-1.5 text-xs font-semibold ${
                  job.isFeatured ? "border-sand-200 text-navy-800/70" : "border-success-600/30 text-success-600"
                }`}
              >
