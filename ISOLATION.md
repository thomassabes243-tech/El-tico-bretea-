# Aislamiento de El Tico Bretea (CR)

Esta rama es `claude/el-tico-bretea-hxg8aq`. No mezclar sus commits con la rama de México.
...
- Esta rama no tiene columna appId ni filtro de tenant. Depende de una base exclusivamente de Costa Rica; la guarda no certifica que datos antiguos no estén contaminados.
- No se ejecutó ninguna migración. El seed dejó de imprimir contraseñas en logs.
...
Las pruebas usan configuraciones sintéticas, no conectan con producción y comprueban rechazo de bases cruzadas, URLs inválidas, schemas distintos, bucket ajeno, consultas Prisma/SQL antes de conectarse a una base ajena.
